<?php
/*Template Name: Industries Landing
*/
get_header();
?>
...
<?php
    ...
    $industries_subpages = get_pages( array( 'child_of' => $post->ID, 'sort_column' => 'post_date', 'sort_order' => 'asc' ) );
?>

<div class="industries-page site-main-cover">
...
<?php if($industries_subpages){ ?>
<section class="industries-section">
    <div class="container">
        <div class="industries-wrap">
            <div class="industries-main flex">
            <?php foreach( $industries_subpages as $industries_subpage ) {
                $practices_short_description = get_field('practices_short_description', $industries_subpage->ID);
                $practices_thumb_image = get_field('practices_thumb_image', $industries_subpage->ID);
            ?>
                <div class="industries-list">
                <?php if(!empty($practices_thumb_image)){ ?>
                    <figure class="industries-thumb object-fit">
                        <a href="<?php echo get_page_link( $industries_subpage->ID ); ?>">
                ...
                <?php } ?>
                    <div class="industries-desc">
                        <h5>
                            <a href="<?php echo get_page_link( $industries_subpage->ID ); ?>">
                                <?php echo $industries_subpage->post_title; ?>
                            </a>
                        </h5>
                        <?php if(!empty($practices_short_description)){ ?>
                                <p><?php echo $practices_short_description; ?></p>
                            <?php } ?>
                        <a href="<?php echo get_page_link( $industries_subpage->ID ); ?>" class="read-btn pos-relative">Learn More<span class="fa-solid fa-arrow-right angle-icon"></span></a>
                    </div>
                </div>
            <?php } ?>
            </div>
            <div class="industries-more-btn hide-tablet hide-desktop">
                <a href="#" class="button">Show More Articles</a>
            </div>
        </div>
    </div>
</section>
